import { Popup } from '@/components/ui/Popup'
import { useTranslation } from 'react-i18next';
import { Button } from '@/components/ui/Button';
import { useModalStoreV2 } from '@/store/use.modal.store';
import { useEffect, useState } from 'react';
import { yupResolver } from '@hookform/resolvers/yup';
import { useForm } from 'react-hook-form';
import { showToast } from '@/utils/toast.util';
import { applyFormServerErrors, showServerMessages } from '@/utils/api.utils';
import GovernorateInputsSkeleton from '../inputs/GovernorateInputsSkeleton';
import GovernorateInputs from '../inputs/GovernorateInputs';
import { validationSchema } from '../../validation';
import { useGovernorate, useUpdateGovernorate } from '@/hooks/api/governorates.hooks';

const EditModal = () => {
  const { t } = useTranslation();
  const { isOpen, getModalData, closeModal } = useModalStoreV2();
  const modalData = getModalData("edit-governorate");

  /* ---------------- Data State ---------------- */
  const [governorate, setGovernorate] = useState(null);
  
  /* ---------------- API ---------------- */
  const { data, isLoading } = useGovernorate(modalData?.id);
  
  const { register, handleSubmit, formState: { errors }, reset, setError } = useForm({
    resolver: yupResolver(validationSchema),
    mode: 'onChange',
  });

  const { 
    mutate: updateGovernorate,
    isPending: isUpdating,
    isSuccess,
    isError,
    error
  } = useUpdateGovernorate();

  /* ---------------- Populate Form ---------------- */
  useEffect(() => {
    if (!data?.data?.data) return;
    
    const row = data.data.data;

    setGovernorate(row);

    // Reset form with row data
    reset({
      name: row.name,
      distance_from_cairo: row.distance_from_cairo,
      is_visible: row.is_visible,
    });
  }, [data, reset]);

  /* ---------------- Submit Handler ---------------- */
  const onSubmit = (data) => {
    if (!governorate?.id) return;
    updateGovernorate({ id: governorate.id, data });
  };

  useEffect(() => {
    if (isSuccess) {
      showToast("success", t("messages:updated_successfully"));
    } if (isError) {
      const res = error.response;
      if (res?.status == 404) {
        showToast("error", t("messages:update_not_found"));
      } if (res?.status == 422) {
        showToast("error", t("messages:validation_error"));
        applyFormServerErrors(res?.data?.errors, setError);
      }
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isSuccess, isError]);

  return (
    <Popup
      isOpen={isOpen("edit-governorate")} 
      closeModal={() => closeModal("edit-governorate")}
      title={t("governorates:modals.edit.title")} 
      description={t("governorates:modals.edit.description")}
    >
      <form onSubmit={handleSubmit(onSubmit)} className='space-y-4'>
        {isLoading ? (
            <GovernorateInputsSkeleton /> 
          ) : (
            <GovernorateInputs 
              register={register} 
              errors={errors} 
            />
        )}
        <div className='flex gap-2'>
          <Button 
            variant="info"
            size="md"
            fullWidth
            isLoading={isLoading || isUpdating}
            disabled={isLoading || isUpdating}
          >
            {isLoading || isUpdating ? t("buttons:isLoading") : t("buttons:edit")}
          </Button>
          <Button 
            variant="cancel"
            size="md"
            fullWidth
            disabled={isLoading}
            onClick={() => closeModal("edit-governorate")}
            type="button"
          >
            {t("buttons:cancel")}
          </Button>
        </div>
      </form>
    </Popup>
  )
}

export default EditModal
