...
export class StudentProfileService {
  ...
  // ── Get ───────────────────────────────────────────
  async getById(userId: number, lang: string) {
    ...
    return this.responseHelper.success(
      this.mapStudentProfile(student),
  ...
  // ── Delete ────────────────────────────────────────
  async delete(userId: number, lang: string) {
    const student = await this.prisma.student.findUnique({
      where: { userId },
    ...
    try {
      this.fileService.deleteDirectory(studentUploadsPath);
    } catch {}

    await this.prisma.student.delete({ where: { userId } });

    return this.responseHelper.success(null, 'student.PROFILE_DELETE_SUCCESS', lang);
  }

  // ── Private mappers ───────────────────────────────
  private mapStudentProfile(student: any) {
    return {
      student:            this.mapStudentMeta(student),
      ...
      documents:          this.mapDocuments(student.documents),
    };
  }

  private mapStudentMeta(student: any) {
    return {
      id: student.id,
  ...
  private mapHousingInfo(student: any) {
    ...
    return {
      housing_type: student.dormType,
      meals: student.requiresMeals,
    };
  }

  private mapDocuments(documents: any[]) {
    return documents.reduce((acc, doc) => {
      const encrypted = encryptFilePath(doc.file_path);
      const fullPath = `${process.env.SERVER_URL}/files/${encrypted}`;

      if (doc.document_type.startsWith('pre_university_certificate')) {
        if (!acc['pre_university_certificate']) {
          acc['pre_university_certificate'] = [];
        }
